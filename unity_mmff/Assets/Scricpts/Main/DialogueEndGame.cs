...
{
    public EndGame scriptAReference;
    public int[] point;
    private int temp = 0;
    ...
    IEnumerator WaitForDataAndCalculate()
    {
        ...
        while (scriptAReference == null || 
            scriptAReference.data == null || 
            scriptAReference.data.Length < 5)
        {
    ...
    void CalculatePoint()
    {
        if (scriptAReference != null && scriptAReference.data != null)
        {
            for (int i = 0; i < 5; i++)
            {
                point[i] = int.Parse(scriptAReference.data[i]);
                Debug.Log(point[i]);
            }
        }
